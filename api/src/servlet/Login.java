...
//	private JSONArray json;
	...
	private static final int WHAT_UPDATE_SIGN_NUM = 51;
	private static final int WHAT_GET_USER_SIGN_NUM = 52;
	private static final int WHAT_GET_USER_INFO = 53;
	private static final int WHAT_SET_COLLEGE = 54;
	private static final int WHAT_SET_MAJOR = 55;
	private static final int WHAT_SET_WECHAT = 56;
...
//						outputStream.write("没有此用户".getBytes("utf-8"));
			...
			case WHAT_ASSIGN:	
				...
				if(!phone.equals("")&&phone!=null){
					if(DaoUsers.Assign(DaoUsers.STABLE_NAME, phone)){
						DaoUsers.insert(DaoUsers.STABLE_NAME,new UserInfoDataBean("ID"+(DaoUsers.getMaxId()+1), password, headurl, phone,"男","中国",0,"",0,"","",0,"","",""));
			...
			case WHAT_PRAISE:
				...
				outputStream.write("thx!".getBytes("utf-8"));
...
//				json = JSONArray.fromObject(questionList); 
			...
			case NEW_WHAT_QQUSER_JUDGE:	
				...
				if(!phone.equals("")&&phone!=null){
					//注册
					if(DaoUsers.Assign(DaoUsers.STABLE_NAME, phone)){
						DaoUsers.insert(DaoUsers.STABLE_NAME,new UserInfoDataBean(username, password, headurl, phone, sex,address,0,"",0,"","",0,"","",""));
...
//						outputStream.write("assign".getBytes("utf-8"));
			...
			case WHAT_UPDATE_SIGN_NUM:
				id = request.getParameter("param1");
				DaoUsers.updateSignNum(DaoUsers.STABLE_NAME,Integer.parseInt(id),1);
				DaoUsers.updateMoney(DaoUsers.STABLE_NAME,Integer.parseInt(id),5);
				break;
				
			case WHAT_GET_USER_SIGN_NUM:
				List<UserInfoBean> userInfoDataBeans = DaoUsers.getUserSignNum(DaoUsers.STABLE_NAME);
				mJsonString = JsonUtils.toJson(userInfoDataBeans);
				outputStream.write(mJsonString.getBytes("utf-8"));
				System.out.println(mJsonString); 
				break;
			case WHAT_GET_USER_INFO:
				outputStream.write(DaoUsers.getUserInfo(DaoUsers.STABLE_NAME, request.getParameter("param1")).getBytes("utf-8"));
				break;
			
				//修改学院
			case WHAT_SET_COLLEGE:
				id = request.getParameter("param1");
				newData = request.getParameter("param2");
				DaoUsers.updateCollege(DaoUsers.STABLE_NAME,Integer.parseInt(id),newData);
				outputStream.write("修改成功".getBytes("utf-8"));
				break;
			...
			case WHAT_SET_MAJOR:
				id = request.getParameter("param1");
				newData = request.getParameter("param2");
				DaoUsers.updateMajor(DaoUsers.STABLE_NAME,Integer.parseInt(id),newData);
				outputStream.write("修改成功".getBytes("utf-8"));
				break;
			
				//修改微信
			case WHAT_SET_WECHAT:
				id = request.getParameter("param1");
				newData = request.getParameter("param2");
				DaoUsers.updateWechat(DaoUsers.STABLE_NAME,Integer.parseInt(id),newData);
				outputStream.write("修改成功".getBytes("utf-8"));
				break;
			case WHAT_GET_BANNER:
				List<NewsBean> BannerBeanList = DbNews.getNews(DbNews.STABLE_NAME_BANNER);
				mJsonString = JsonUtils.toJson(BannerBeanList);
				System.out.println(mJsonString); 
				try {
					outputStream.write(mJsonString.getBytes("utf-8"));
			...
			case WHAT_GET_NEWS:
				List<NewsBean> NewsBeanList = DbNews.getNews(DbNews.STABLE_NAME_NEWS);
